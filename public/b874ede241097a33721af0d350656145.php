<?php /*a:4:{s:82:"/www/wwwroot/saas.kf.renloong.com/plugin/ycDigitalHuman/public/pc/User/dealer.html";i:1729243919;s:83:"/www/wwwroot/saas.kf.renloong.com/plugin/ycDigitalHuman/public/pc/modules/main.html";i:1729244028;s:85:"/www/wwwroot/saas.kf.renloong.com/plugin/ycDigitalHuman/public/pc/modules/header.html";i:1729232021;s:83:"/www/wwwroot/saas.kf.renloong.com/plugin/ycDigitalHuman/public/pc/modules/menu.html";i:1728979414;}*/ ?>
<!DOCTYPE html>
...
<head>
    ...
    <script>
        const __YCCONFIG = {
            ...
            web_signin: '<?php echo htmlentities($system['web_signin']); ?>',
...
<div class="flex flex-center app-init-none py-10" v-if="USERINFO">
    <div class="p-4 flex-2 flex flex-center">
        <div class="text-center" v-if="dealer.state === null">
            <el-icon color="var(--el-color-primary)" size="120">
                <Lock />
            </el-icon>
            <div class="h1 font-weight-600 my-10 text-primary">申请开通分销权限</div>
            <div class="pt-4 h9 text-info">申请开通分销权限后，您可以通过分享链接或二维码的方式推广本平台，推广成功后，您将获得相应的分销奖励
            </div>
            <div class="pt-4 h8 text-error font-weight-600"
                v-if="dealer.config.dealer_apply_refund === 'no' && dealer.config.dealer_apply_price > 0">{{
                dealer.config.dealer_apply_refund_text }}</div>
            <div class="pt-10">
                <el-button v-if="dealer.config.dealer_apply_price > 0" :loading="loading" size="large" type="primary"
                    @click="applyDealer">支付(￥{{
                    dealer.config.dealer_apply_price }}元)申请开通</el-button>
                <el-button v-else-if="dealer.config.dealer_apply_price > 0" size="large" type="primary"
                    disabled>联系客服申请开通</el-button>
                <el-button v-else size="large" @click="applyDealer" :loading="loading" type="primary">申请开通</el-button>
            </div>
        </div>
        <div class="text-center" v-else-if="dealer.state === 0">
            <el-icon color="var(--el-color-primary)" size="120">
                <Lock />
            </el-icon>
            <div class="h1 font-weight-600 my-10 text-primary">等待后台审核开通分销权限</div>
            <div class="pt-4 h9 text-info">申请开通分销权限后，您可以通过分享链接或二维码的方式推广本平台，推广成功后，您将获得相应的分销奖励
            </div>
            <div class="pt-4 h8 text-error font-weight-600"
                v-if="dealer.config.dealer_apply_refund === 'no' && dealer.config.dealer_apply_price > 0">{{
                dealer.config.dealer_apply_refund_text }}</div>
            <div class="pt-10">
                <el-button type="info">等待后台审核</el-button>
            </div>
        </div>
        <div class="p-4 w-50 mx-auto" v-else-if="dealer.state === 1">
            <div class="flex bg-white rounded-4 p-2 mb-6">
                <div class="flex-1 text-center py-4 rounded-4">
                    <div class="text-primary font-weight-600 h8">{{ dealer.user.people_num }}</div>
                    <div class="h10 text-info mt-2">邀请人数</div>
                </div>
                <div class="flex-1 text-center py-4 rounded-4">
                    <div class="text-primary font-weight-600 h8">{{ dealer.user.order_price }}</div>
                    <div class="h10 text-info mt-2">订单总额</div>
                </div>
                <div class="flex-1 text-center py-4 rounded-4">
                    <div class="text-primary font-weight-600 h8">{{ dealer.user.dealer_price }}</div>
                    <div class="h10 text-info mt-2">累计收益</div>
                </div>
            </div>
            <div class="bg-white p-6 rounded-6 flex flex-center mb-6">
                <div class="flex-1 flex">
                    <div class="mr-6 text-danger">
                        <div class="h10">订单</div>
                        <div class="pt-4 h2 font-weight-600">{{ dealer.config.dealer_proportion }}%</div>
                    </div>
                    <div class="flex-1" v-if="dealer.config.dealer_level >= 1">
                        <div class="h10">一级</div>
                        <div class="pt-4 h6 font-weight-600">{{ dealer.config.dealer_proportion_level_1 }}%</div>
                    </div>
                    <div class="flex-1" v-if="dealer.config.dealer_level >= 2">
                        <div class="h10">二级</div>
                        <div class="pt-4 h6 font-weight-600">{{ dealer.config.dealer_proportion_level_2 }}%</div>
                    </div>
                    <div class="flex-1" v-if="dealer.config.dealer_level >= 3">
                        <div class="h10">三级</div>
                        <div class="pt-4 h6 font-weight-600">{{ dealer.config.dealer_proportion_level_3 }}%</div>
                    </div>
                </div>
            </div>
            <div class="bg-white rounded-6 p-6 flex flex-column">
                <div class="flex flex-center pb-6 border-bottom">
                    <div class="flex-1 h6 font-weight-600 text-info">分销链接</div>
                </div>
                <div class="py-4 flex flex-y-center">
                    <div class="bg rounded-round flex-1 px-4 py-2 text-break-all">{{ dealer.link }}</div>
                    <div class="p-4 font-weight-600 text-primary h9 pointer" @click="copyLink">复制</div>
                </div>
                <div class="text-center mb-4 pt-4">
                    <img v-if="dealer.qrcode" :src="dealer.qrcode" class=" border rounded-4" width="200" height="200" />
                </div>
                <div class="text-info h9 mb-4">
                    如果客户通过该链接注册本平台 ， 该客户将永久綁定为您的下级客户 ， 该客户在该平台的所有消订单会按照当前分销比例返还到您的账户中
                </div>
                <div class="text-danger h9 mb-4">
                    分销奖励计算公式：订单金额 * 订单比例 * 分销比例
                </div>
                <div class="text-info h9">
                    如：订单金额100元，订单比例为20%，一级分销比例为50%，则分销奖励为：100 * 20% * 50% = 10元
                </div>
            </div>
        </div>
        <div class="text-center" v-else-if="dealer.state === 2">
            <el-icon color="var(--el-color-primary)" size="120">
                <InfoFilled />
            </el-icon>
            <div class="h1 font-weight-600 my-10 text-primary">分销权限异常</div>
            <div class="pt-4 h9 text-info">当前分销权限已被冻结，你不可进行操作，请联系管理员</div>
            <div class="pt-10">
                <el-button type="danger" size="large">权限异常</el-button>
            </div>
        </div>
    </div>
</div>
...
<script src="/app/ycDigitalHuman//pc/User/dealer.js"></script>
