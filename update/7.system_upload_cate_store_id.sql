...
BEGIN
	...
	THEN
		ALTER TABLE `yc_system_upload_cate` ADD COLUMN `store_id` int(11) NULL COMMENT '租户ID' AFTER `is_system`;
	END IF;
	IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = 'yc_system_upload_cate' AND column_name = 'store_id')
	THEN
		ALTER TABLE `yc_system_upload_cate` MODIFY COLUMN `store_id` int(11) NULL COMMENT '租户ID' AFTER `is_system`;
	END IF;
END;
